...
export default function PreMigrationHub() {
  const router = useRouter();
  const [tokens, setTokens] = useState([]);
  const [topPriority, setTopPriority] = useState([]);
  const [loading, setLoading] = useState(true);
  const [intervalSetting, setIntervalSetting] = useState(2);
  const [expandedLogId, setExpandedLogId] = useState(null);

  useEffect(() => {
    const savedInterval = localStorage.getItem('dexlive_pre_interval');
    if (savedInterval) {
      setIntervalSetting(parseInt(savedInterval, 10));
    }
  }, []);

  const fetchPreMigrationData = async () => {
    try {
      const res = await fetch('/api/scanner/pre-migration');
      if (res.ok) {
      ...
      console.error("Failed to fetch pre-migration tokens:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchPreMigrationData();
    const pollInterval = setInterval(fetchPreMigrationData, intervalSetting * 60 * 1000);
          ...
          <div className="flex items-center justify-between border-b border-emerald-900 pb-5">
            <div>
              ...
              <h1 className="text-3xl font-black tracking-tight text-white flex items-center gap-3">
                ...
                PHASE 01: PRE-MIGRATION HUB (PUMP.FUN)
              </h1>
              <p className="text-slate-400 text-xs mt-1">
                Live hybrid pipeline inspection. Active scan interval: <strong className="text-emerald-400">{intervalSetting} MINUTES</strong>.
              </p>
            </div>
            <button
              onClick={fetchPreMigrationData}
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-300 uppercase tracking-widest mb-4">
              &gt; TOP_03_PRIORITY_A_CANDIDATES (PRE-MIGRATION)
            </h2>
            {loading ? (
              ...
              <div className="bg-slate-950 border border-emerald-900/60 rounded-xl p-6 text-center text-slate-500 text-xs">
                Awaiting tokens clearing AI smart money & sniper flush threshold in current scan cycle...
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div key={coin.mint || idx} className="bg-slate-950 border border-emerald-500/50 rounded-xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.15)] relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded bg-emerald-950 border border-emerald-800 flex items-center justify-center font-bold text-emerald-400 text-sm">
                        {coin.symbol ? coin.symbol.slice(0, 3) : 'MEME'}
                      </div>
                      <div>
                    ...
                    <div className="space-y-1.5 text-xs text-slate-300 mb-4 bg-black p-3 rounded border border-emerald-900/40">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-emerald-400">{coin.bonding_curve}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Buy/Sell Ratio:</span>
                        <span className="font-bold text-emerald-300">{coin.buy_sell_ratio}:1</span>
                      </div>
                      <div className="flex justify-between">
          ...
          {/* ALL PRE-MIGRATION CANDIDATES & LIVE PROCESS LOG */}
          <div className="bg-slate-950 border border-emerald-900/80 rounded-xl overflow-hidden shadow-xl">
            <div className="p-5 border-b border-emerald-900 flex items-center justify-between">
              <h3 className="font-bold text-white text-sm">&gt; LIVE_PRE_MIGRATION_CANDIDATES_FEED</h3>
              <span className="text-xs text-emerald-400">{tokens.length} active bonding curve items</span>
            </div>

            {loading ? (
              <div className="p-10 text-center text-slate-500 text-xs">Loading pre-migration telemetry...</div>
            ) : tokens.length === 0 ? (
              <div className="p-10 text-center text-slate-500 text-xs">No records found. Waiting for Helius cron sync.</div>
            ) : (
                  ...
                  <div key={token.mint || index} className="p-5 hover:bg-slate-900/40 transition">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded bg-emerald-950 border border-emerald-800 flex items-center justify-center font-bold text-emerald-400 text-sm">
                          {token.symbol ? token.symbol.slice(0, 3) : 'M'}
                        </div>
                        <div>
                      ...
                      <div className="flex flex-wrap items-center gap-6 text-xs">
                        <div>
                          ...
                          <p className="font-bold text-emerald-400 mt-0.5">{token.bonding_curve}%</p>
                        </div>
                        <div>
                          <p className="text-[10px] text-slate-500">BUY/SELL</p>
                          <p className="font-bold text-emerald-300 mt-0.5">{token.buy_sell_ratio}:1</p>
                        </div>
                        <div>
                    ...
                    {expandedLogId === (token.mint || index) && (
                      <div className="mt-4 pt-4 border-t border-emerald-900/60 bg-black rounded-lg p-4">
                        <h5 className="text-[11px] font-bold text-emerald-400 uppercase tracking-widest mb-3">
                          &gt; HYBRID_PIPELINE_EXECUTION_LOG [SYSTEMATIC + AI]
                        </h5>
