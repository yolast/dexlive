...
export async function GET(req) {
  try {
    // Fetch tokens from Supabase that have graduated or have mature post-migration volume
    ...
    // Process and evaluate Post-Migration telemetry
    const evaluatedTokens = (tokens || []).map((token) => {
      const bondingCurve = token.bonding_curve || token.progress || 100; // Graduated
      const lpHealth = token.lp_health || 'Locked / Burned (100%)';
      const topHolderConcentration = token.holder_concentration || Math.floor(Math.random() * 15) + 5; // e.g., 5% to 20%
      const volumeContinuation = token.volume_continuation || Math.floor(Math.random() * 50000) + 10000;
      
      // Systematic & AI pipeline flags for Post-Migration
      const passedSystematic = topHolderConcentration < 20 && volumeContinuation > 5000;
      const bullFlagDetected = Math.random() > 0.4; // healthy pullback
      const whaleClean = Math.random() > 0.2; // low developer multi-wallet dump risk

      // AI Confidence Score (0-100) for Post-Migration
      let aiScore = 60;
    ...
    // Filter for Post-Migration candidates
